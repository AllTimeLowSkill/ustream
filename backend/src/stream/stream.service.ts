...
export class StreamService {
  constructor(
    ...
    @InjectRepository(StreamInfo)
    private readonly streamInfoRepository: Repository<StreamInfo>,
  ...
  createStreamData = async (data: any) => {
    ...
    const streamInfoCreated = await this.streamInfoRepository.findOne({
      where: {
    ...
    if (!streamInfoCreated) {
      const streamInfo = await this.streamInfoRepository.save({
        streamKey: stream.streamKey,
        categoryId: '',
      })
      return {
  ...
  updateStreamCategory = async (id: string, streamKey: string) => {
    const isMatch = await this.streamInfoRepository.update(
      { streamKey },
      {
        categoryId: id,
      },
    ...
    if (isMatch) {
      return await this.streamInfoRepository.findOne({
        where: {
          streamKey,
        },
      })
  ...
  getStreams = async () => {
    ...
    for (const stream of streams) {
      const data = await this.streamInfoRepository.findOne({
        where: {
          streamKey: stream.streamKey,
        },
      })
